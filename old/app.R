...
server <- function(input, output, session) {
  ...
  output$p1 <- renderPlotly({
    req(w1$get_triggered())
    if(w1$get_triggered() == TRUE) ggplotly(p1)
  })
  output$p2 <- renderPlotly({
    req(w2$get_triggered())
    if(w2$get_triggered() == TRUE) ggplotly(p2)
  })
  output$p3 <- renderPlotly({
    req(w3$get_triggered())
    if(w3$get_triggered() == TRUE) ggplotly(p3)
  })
  output$p4 <- renderPlotly({
    req(w4$get_triggered())
    if(w4$get_triggered() == TRUE) ggplotly(p4)
  })
  output$p5 <- renderPlotly({
    req(w5$get_triggered())
    if(w5$get_triggered() == TRUE) ggplotly(p5)
  })
  output$p6 <- renderPlotly({
    req(w6$get_triggered())
    if(w6$get_triggered() == TRUE) ggplotly(p6)
  })
